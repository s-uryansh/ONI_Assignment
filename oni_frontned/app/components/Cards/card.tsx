...
export const InteractiveGradient = ({
  ...
  const [resolvedGlowFallbackColor, setResolvedGlowFallbackColor] = useState("#00000"); // Renamed for clarity
  ...
  useEffect(() => {
    ...
    if (!backgroundColor || !backgroundColor.startsWith("#") && !backgroundColor.startsWith("rgb")) {
        ...
        const updateColor = () => {
          ...
          setResolvedGlowFallbackColor(isDark ? "#000" : "#000"); // Black for dark, white for light as default
        };
  ...
  return (
    <div
      ...
      className={cn(
        ...
        interactive-gradient-card text-foreground`, // removed 'border' here
        backgroundColor || 'bg-black dark:bg-black',
        className
      )}
      style={{
        ...getBackgroundStyle(),
      ...
      <style>
        {`
          .interactive-gradient-card::before {
            /* disable the pseudo-element border to remove the white/gradient border */
            display: none;
          }
        `}
      </style>
