...
interface SparkleNavbarProps {
  /**
   * An array of nav items. Each entry can be either a string (treated as label)
   * or a NavItem object with optional path.
   * @example ['Home', 'About'] or [{ label: 'Home', path: '/' }]
   */
      ...
      <nav className="navigation-menu" ref={navRef}>
        <ul>
          {items.map((item, index) => {
            const itemObj: NavItem =
              typeof item === "string" ? { label: item, path: `/${item.toLowerCase()}` } : item;
            const key = `${itemObj.path ?? itemObj.label}-${index}`;
            return (
              <li key={key} className={index === activeIndex ? "active" : ""}>
                <button
                  ref={(el) => {
